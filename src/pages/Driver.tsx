...
export default function Driver() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Pastikan kolom date kosong menjadi null
    const safeData = {
      ...dataToSend,
    ...
    if (!id || id === 0) {
      ...
      const { error } = await supabase.from("driver").update(safeData).eq("id", id);
      if (error) {
        ...
        <table className="min-w-full table-auto border border-gray-300 text-sm">
          <thead>
            <tr className="bg-gray-400 text-white">
              <th rowSpan={2} className="border p-2 text-center align-middle">✔</th>
