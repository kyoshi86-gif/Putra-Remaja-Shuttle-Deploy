...
export default function KasKasir() {
  ...
  const fetchEntities = async () => {
    ...
    if (error) {
      ...
      setEntities(
        data as { id: string; kode: string; nama: string; tipe: string }[],
      );
    }
  };
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const fetchSaldoAwalHistori = async (
        startDate: string,
      ): Promise<number> => {
        let query = supabase
      ...
      const premiDriver =
        premiData?.reduce((t, r) => t + (r.nominal || 0), 0) || 0;
      ...
      const potonganDriver =
        potonganData?.reduce((t, r) => t + (r.nominal || 0), 0) || 0;
      ...
      const perpalDriver =
        perpalData?.reduce((t, r) => t + (r.nominal || 0), 0) || 0;
      ...
      const realisasiSaku =
        realSakuData?.reduce((t, r) => t + (r.nominal || 0), 0) || 0;

      // Kas Masuk (debet)
      // Termasuk:
      // 1. Transaksi kas biasa (sumber_tabel NULL)
      // 2. Sisa / Kembali Kasbon
      // 3. Sisa / Kembali Realisasi Saku
      const { data: masukData, error: masukError } = await supabase
        .from("kas_harian")
        .select("nominal")
        .eq("jenis_transaksi", "debet")
        .or(
          "sumber_tabel.is.null," +
          "sumber_tabel.eq.kasbon_realisasi_sisa," +
          "sumber_tabel.eq.realisasi_saku_sisa"
        )
        .gte("tanggal", startDate)
        .lte("tanggal", endDate)
        .eq("entity_id", targetEntity);
      ...
      if (masukError)
        console.error("❌ Kas Masuk fetch error:", masukError.message);
      const kasMasuk =
        masukData?.reduce((t, r) => t + (r.nominal || 0), 0) || 0;
      ...
      const kasKeluar =
        keluarData?.reduce((t, r) => t + (r.nominal || 0), 0) || 0;
      ...
      if (kasbonError)
        console.error("Kasbon fetch error:", kasbonError.message);
      const kasbon =
        kasbonData?.reduce((t, r) => t + (r.jumlah_kasbon || 0), 0) || 0;
      ...
      const realisasiKasbon =
        realKasbonData?.reduce((t, r) => t + (r.nominal || 0), 0) || 0;
      ...
      if (etollError)
        console.error("Biaya EToll fetch error:", etollError.message);
      const biayaEtoll =
        etollData?.reduce((t, r) => t + (r.biaya_etoll || 0), 0) || 0;
  ...
  return (
    <div
      id="print-root"
      className="p-6 max-w-3xl mx-auto bg-white min-h-screen"
    >
      ...
      {loading ? (
        ...
        >
          ...
          <div className="bg-gray-100 rounded-xl p-4 shadow-inner">
            <h2 className="font-bold mb-3 text-gray-800 text-lg">
            ...
            <div className="flex justify-between py-1">
              ...
              <span className="font-semibold text-blue-700">
                {formatCurrency(summary.potonganDriver)}
              </span>
            </div>
          ...
          <div className="bg-gray-100 rounded-xl p-4 shadow-inner">
            ...
            <div className="flex justify-between py-1">
              ...
              <span className="font-semibold text-red-700">
                {formatCurrency(summary.uangSaku)}
              </span>
            </div>
            ...
            <div className="flex justify-between py-1">
              ...
              <span className="font-semibold text-red-700">
                {formatCurrency(summary.premiDriver)}
              </span>
            </div>
            ...
            <div className="flex justify-between py-1">
              <span>Perpal</span>
              <span className="font-semibold text-red-700">
                {formatCurrency(summary.perpalDriver)}
              </span>
            </div>
          ...
          <div className="bg-white rounded-xl p-4 border shadow-sm">
            ...
            <div className="flex justify-between py-1">
              ...
              <span className="font-semibold text-purple-700">
                {formatCurrency(summary.realisasiSaku)}
              </span>
            </div>
            ...
            <div className="flex justify-between py-1">
              ...
              <span className="font-semibold text-purple-700">
                {formatCurrency(summary.realisasiKasbon)}
              </span>
            </div>
            ...
            <div className="flex justify-between font-bold border-t border-gray-300 mt-2 pt-2 text-gray-900">
              ...
              <span className="text-purple-800">
                {formatCurrency(
                  summary.realisasiSaku + summary.realisasiKasbon,
                )}
              </span>
            </div>
          </div>
